		    ...
		    int kill_parity_nr, int write_type)
{
	...
	d_rank_t		kill_parity_rank;
	int			kill_parity_rank_num = 1;
	...
	if (kill_parity_nr > 0) {
		 ...
		 */
		get_killing_rank_by_oid(arg, oid, 1, 0, &kill_parity_rank,
					&kill_parity_rank_num);
		rebuild_single_pool_target(arg, kill_parity_rank, -1, false);
	}
	...
	if (kill_parity_nr > 0)
		reintegrate_single_pool_target(arg, kill_parity_rank, -1);
}
