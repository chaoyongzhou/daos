			 ...
			 int data_tgt_nr, int *parities)
{
	...
	shard_idx = grp_size * (grp_idx + 1) - 1;
		      ...
		      struct daos_oclass_attr *oca, daos_obj_list_t *args)
{
	...
	int			grp_size = obj_ec_parity_tgt_nr(oca);
	unsigned int		first;
	int			idx;
	int			i;
	...
	obj_ec_get_parity_shards(obj, grp_idx, obj_ec_data_tgt_nr(oca),
				 parities);
	...
	grp_size = obj_ec_parity_tgt_nr(oca);
	/* choose one randomly from the parities */
	idx = random() % grp_size;
	for (i = 0; i < grp_size; i++, idx = (idx + 1) % grp_size) {
		/* let's skip the rebuild shard for non-update op */
		shard = parities[idx];
	...
	grp_size = obj_ec_data_tgt_nr(oca);
